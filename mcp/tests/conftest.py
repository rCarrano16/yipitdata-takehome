...
def _no_db_session() -> Iterator[None]:
    """Stand in for app.db.session_scope without opening a database connection.
...
def stub_session_scope(monkeypatch: pytest.MonkeyPatch) -> None:
    """Point every tool's session_scope at the no-op session, so no test hits a DB."""
    monkeypatch.setattr(server, "session_scope", _no_db_session)
