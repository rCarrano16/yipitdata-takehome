...
- **MCP server** (`mcp/server.py`): a FastMCP server exposes the same data as
  seven read-only tools and two prompts for AI agents, over the Model Context
  Protocol on stdio.
...
| MCP tools an LLM can use naturally | Seven intent-shaped, read-only tools with LLM-facing docstrings, typed Pydantic return models (so FastMCP publishes an output schema), and truthful read-only annotations, plus two prompts that chain the tools into common analyst workflows. |
